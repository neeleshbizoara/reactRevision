  ...
  return !resInfo || resInfo.length === 0 ? (
    ...
    ">
      <h2 className="font-bold text-3xl text-sky-700">{resInfo.data.cards[0]?.card?.card?.text}</h2>
        ...
        {resInfo.data.cards[4]?.groupedCard?.cardGroupMap?.REGULAR?.cards.map(item => item.card.card.itemCards ? 
          <div key="" className="flex-wrap">    
                ...
                (card) => {
                  ...
                  return (
                      <div className="w-[200px] bg-gray-500 m-4" key={id}>
                        ...
                        <button type="button" class="m-2 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Add</button>
                        </div>
                      </div>
                      </div>
                  );
